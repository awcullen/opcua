...
func TestStruct(t *testing.T) {
	cases := []struct {
		...
		{
			&ua.PublishResponse{
				ResponseHeader:           ua.ResponseHeader{Timestamp: time.Date(1601, time.January, 01, 12, 0, 0, 0, time.UTC), RequestHandle: 1000085, StringTable: []string{}},
			...
			&ua.PublishResponse{},
			[]byte{
				0x00, 0xe0, 0x34, 0x95, 0x64, 0x00, 0x00, 0x00, 0x95, 0x42, 0x0f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // header
				...
				0x00, 0x00, 0x00, 0x00, // diag infos
				0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // results
				0x00, 0x00, 0x00, 0x00, // diag infos
			},
		},
		{
			&ua.CreateSessionRequest{RequestHeader: ua.RequestHeader{Timestamp: time.Date(1601, time.January, 01, 12, 0, 0, 0, time.UTC)}, ClientDescription: ua.ApplicationDescription{DiscoveryURLs: []string{}}},
			&ua.CreateSessionRequest{},
			[]byte{
				0x00, 0x00, 0x00, 0xe0, 0x34, 0x95, 0x64, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00,
				0xff, 0xff, 0xff, 0xff,
				0xff, 0xff, 0xff, 0xff,
				0xff, 0xff, 0xff, 0xff,
				0xff, 0xff, 0xff, 0xff,
...
func TestMatrix(t *testing.T) {
	...
	for _, c := range cases {
		buf := &bytes.Buffer{}
		enc := ua.NewBinaryEncoder(buf, ua.NewEncodingContext())
		if err := enc.WriteVariant(c.in); err != nil {
			t.Fatal(err)
		}
		assert.DeepEqual(t, buf.Bytes(), c.bytes)
...
func TestStructWithMatrix(t *testing.T) {
